...
namespace idpMultiTenant1.FIDO2
{
    public class Fido2Store
    {
        ...
        {
            return await _applicationDbContext.FidoStoredCredentials.Where(c => c.UserName == username).ToListAsync();
        }

        public async Task RemoveCredentialsByUserNameAsync(string username)
        {
            var items = await _applicationDbContext.FidoStoredCredentials.Where(c => c.UserName == username).ToListAsync();
            if (items != null)
            {
                foreach (var fido2Key in items)
                {
                    _applicationDbContext.FidoStoredCredentials.Remove(fido2Key);
                };
        ...
        public async Task<FidoStoredCredential?> GetCredentialByIdAsync(byte[] id)
        {
            ...
            var cred = await _applicationDbContext.FidoStoredCredentials
        ...
        public Task<ICollection<FidoStoredCredential>> GetCredentialsByUserHandleAsync(byte[] userHandle)
        {
            return Task.FromResult<ICollection<FidoStoredCredential>>(
                _applicationDbContext
                    .FidoStoredCredentials.Where(c => c.UserHandle != null && c.UserHandle.SequenceEqual(userHandle))
                    .ToList());
        }
        ...
        public async Task AddCredentialToUserAsync(Fido2User user, FidoStoredCredential credential)
        {
            ...
            _applicationDbContext.FidoStoredCredentials.Add(credential);
            await _applicationDbContext.SaveChangesAsync();
        }
